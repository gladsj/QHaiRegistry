...
EPISODE_HISTORY_CTE AS (
  SELECT
      ...
      LISTAGG(
        TO_CHAR(
          (FROM_TZ(CAST(EH.HISTORY_START_DT AS TIMESTAMP), 'US/Central') AT TIME ZONE 'UTC'),
      ...
      LISTAGG(
        TO_CHAR(
          (FROM_TZ(CAST(EH.HISTORY_END_DT AS TIMESTAMP), 'US/Central') AT TIME ZONE 'UTC'),
          'YYYY-MM-DD"T"HH24:MI:SS"Z"'
        ),
        '{{delimiter}}'
...
SELECT DISTINCT
       ...
       TO_CHAR(
         (FROM_TZ(CAST(E.START_DATE AS TIMESTAMP), 'US/Central') AT TIME ZONE 'UTC'),
         'YYYY-MM-DD"T"HH24:MI:SS"Z"'
       ) AS START_DATE,
       TO_CHAR(
         (FROM_TZ(CAST(E.END_DATE AS TIMESTAMP), 'US/Central') AT TIME ZONE 'UTC'),
         'YYYY-MM-DD"T"HH24:MI:SS"Z"'
       ) AS END_DATE,
